...
fun main() { 
    ...
    val table = connection.getTable(name = "users", create = true) { User(name = "", age = 0) } //Create table with name and function to provide a base instance
    ...
    assertEquals(table.insert(User(name = "Max", age = 25)).isSucces())
    
    val result = table.select().list() //Will read all table rows, create an User instance for every row and then put into a List
...
fun main() {
	...
	table.select(where = (property(User::name) isEqualTo value("Max")) and (property(User::age).isBetween(value(18), value(30))))
}
```
...
##### property
A property node will reference a database column. You can simply create a property node by calling `property(YourClass::yourProperty)` (Referencing the kotlin property).
This way the references will automatically infer the property type and always stay synced to your Classes. You can also pass multiple property references for column reference chains, e.g. `property(Book::author, Author::name)`.
...
For example `createCondition(property(User::age) + " isEqualTo " + value(1))` can be written as `property(User::age) isEqualTo value(1)`, because all default operations have (infix) functions available.
...
Foe example, to get the uppercase value of a string using the SQL `uppper` function you can do `"upper"(property(User::name))`. This will be translated to `upper("users"."name")` (This will work for both value and property nodes).

For the most basic SQL functions there are also default extension functions available. Instead of `"upper"(property(User::name))` you can write `property(User::name).uppercase()`. These extension functions are type sensitive, so you can only call the uppercase extension function on String properties.
...
fun main() {
    ...
    val names = table.selectValue(property(User::name), where = property(User::age) isGreaterThan value(5)).list() //This will only select the name column. You can also specify conditions and all parameters that the normal select supports
    
    table.update(property(User::name) to value("Test"), where = property(User::age) isGreaterThan value(5)) //Update only the name column (You can update multiple columns at once as well). You can also optionally specify a condition here 
}
```

### Multi-Column access
You can also select only specific columns and still let them automatically be converted to you Class:

```kotlin
fun main() {
    //Your connection declaration...
    
    val user = table.select(property(User::name), property(User::email)).first()
...
fun main() {
    ...
    val user = table.select(property(User::name).uppercase()).first()
...
fun main() {
    val connection = PostgresConnection("localhost:5432/test", user = "test", password = "test")
    val authorTable = connection.getTable(name = "authors", create = true) { Author(name = "") }
    val bookTable = connection.getTable(name = "authors", create = true) { Book(name = "", author = Author(name = "")) }
    
    val author = authorTable.insert(Author(name = "Test Author")).value!!
    val book = bookTable.insert(Book(name = "Test Book", author = author)).value!!
    
    assertEquals(author, bookTable.selectValue(property(Book::author)).first())
    assertEquals("Test Author", bookTable.select(property(Book::author, Author::name)).first())
}
```
...
interface UserTable : Table<User> {
    ...
    fun getUserByName(@Condition name: String): User? 
    ...
    fun createUser(@Parameter name: String, @Parameter age: Int): User
    ...
    //Custom function
    fun getAdults() = select(where = property(User::age) isGreaterThan value(18))
    ...
    fun getOlderThan(@Condition(name = "age", operation = " > ") minAge: Int): List<User>
...
fun main() {
    ...
    val table = connection.getTable<_, UserTable>(name = "users", create = true) { User(name = "", age = 0) }
