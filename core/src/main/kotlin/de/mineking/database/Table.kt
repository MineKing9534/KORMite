...
interface DefaultTable<T: Any> : Table<T> {
	...
	fun selectRowCount(where: Node<Boolean> = Conditions.EMPTY): Int
	fun <C> selectValue(target: Node<C>, type: KType, where: Node<Boolean> = Conditions.EMPTY, order: Order? = null, limit: Int? = null, offset: Int? = null): QueryResult<C>

	fun select(vararg columns: Node<*>, where: Node<Boolean> = Conditions.EMPTY, order: Order? = null, limit: Int? = null, offset: Int? = null): QueryResult<T>
	fun select(vararg columns: KProperty<*>, where: Node<Boolean> = Conditions.EMPTY, order: Order? = null, limit: Int? = null, offset: Int? = null): QueryResult<T> = select(columns = columns.map { property(it) }.toTypedArray(), where, order, limit, offset)
	fun select(where: Node<Boolean> = Conditions.EMPTY, order: Order? = null, limit: Int? = null, offset: Int? = null): QueryResult<T> = select(columns = emptyArray<KProperty<*>>(), where, order, limit, offset)

	fun update(obj: T): UpdateResult<T>
	fun update(vararg columns: Pair<Node<*>, Node<*>>, where: Node<Boolean> = Conditions.EMPTY): UpdateResult<Int>

	fun insert(obj: T): UpdateResult<T>
	fun upsert(obj: T): UpdateResult<T>

	fun delete(where: Node<Boolean> = Conditions.EMPTY): Int
