...
@Suppress("UNCHECKED_CAST")
val <C> Node<C>.length where C: Iterable<*> get() = "array_length"(this, unsafe("1")) as Node<Int>

infix fun <T, C: Iterable<T>> Node<C>.contains(other: Node<T>) = createCondition(other + " = any(" + this + ")")

infix fun Node<String>.matches(pattern: Node<String>) = createCondition(this + " ~ " + pattern)
infix fun Node<String>.matches(pattern: String) = createCondition(this + " ~ '" + pattern + "'")

infix fun Node<String>.matchesIgnoreCase(pattern: Node<String>) = createCondition(this + " ~* " + pattern)
infix fun Node<String>.matchesIgnoreCase(pattern: String) = createCondition(this + " ~* '" + pattern + "'")
