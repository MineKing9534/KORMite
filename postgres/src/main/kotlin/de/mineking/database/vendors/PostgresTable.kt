	...
	override fun createTable() {
		...
		fun <C> formatColumn(column: ColumnData<T, C>): String {
			...
			return """
				...
				${ if (root.autogenerate) root.property.getDatabaseAnnotation<AutoGenerate>()?.generator?.takeIf { it.isNotBlank() }?.let { " default $it" } ?: " generated by default as identity" else "" }
			"""
		}
		...
		structure.manager.driver.useHandleUnchecked { it.createUpdate("""
			create table if not exists ${ structure.name } (
				...
				${ if (keys.isNotEmpty()) ", primary key(${ keys.map { "\"${ it.name }\"" }.joinToString() })" else "" }
